...
class OrganizeCardStackState extends State<OrganizeCardStack> with TickerProviderStateMixin {
  ...
  Widget _buildCurrentCard(Size size) {
    ...
    return AnimatedBuilder(
        ...
        builder: (context, child) {
          final isHorizontalDrag = _isDragging && (_currentDirection == _SwipeDirection.left || _currentDirection == _SwipeDirection.right);
          final currentOffset = _isDragging ? (isHorizontalDrag ? Offset.zero : _dragOffset) : (_dismissController.isAnimating ? _dismissAnimation.value : (_snapBackController.isAnimating ? _snapBackAnimation.value : Offset.zero));
          final currentRotation = _isDragging ? (isHorizontalDrag ? 0.0 : _rotation) : (_dismissController.isAnimating ? _rotation : (_snapBackController.isAnimating ? _snapBackRotation.value : 0.0));
          return Transform(
  ...
  void _onScaleUpdate(ScaleUpdateDetails details) {
    ...
    final newOffset = _dragOffset + details.focalPointDelta;
    final newDirection = _computeDirectionFrom(newOffset);

    if (newDirection == _SwipeDirection.left || newDirection == _SwipeDirection.right) {
      _dragOffset = newOffset;
      _currentDirection = newDirection;
      final size = MediaQuery.sizeOf(context);
      if (newDirection == _SwipeDirection.left && -_dragOffset.dx > size.width * _dismissThresholdHorizontal) {
        _isDragging = false;
        _switchToNext();
      } else if (newDirection == _SwipeDirection.right && _dragOffset.dx > size.width * _dismissThresholdHorizontal) {
        _isDragging = false;
        _switchToPrevious();
      }
    } else if (newDirection == _SwipeDirection.up || newDirection == _SwipeDirection.down) {
      setState(() {
        _dragOffset = newOffset;
  ...
  _SwipeDirection? _computeDirectionFrom(Offset offset) {
  ...
  void _switchToNext() {
    ...
    setState(() {
      ...
      _rotation = 0;
      _currentDirection = null;
      widget.indexNotifier.value = targetIndex.clamp(0, entries.length);
    });
  }
  ...
  void _switchToPrevious() {
    ...
    if (targetIndex >= 0) {
      setState(() {
        ...
        _rotation = 0;
        _currentDirection = null;
        widget.indexNotifier.value = targetIndex;
      });
    } else {
      setState(() {
        _dragOffset = Offset.zero;
